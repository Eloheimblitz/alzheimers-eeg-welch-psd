...
\begin{itemize}[leftmargin=2em]
    ...
    \item Implement comparative modeling with ShallowConvNet, DeepConvNet, SCCNet, FBCNet, and EEGNet.
...
\begin{itemize}
    \item Current status: Phase 1 completed with overlap-comparison extension
    \item Completed scope: Dataset inspection, metadata creation, preprocessing, artifact rejection, epoch segmentation (0\% and 50\% overlap), Welch PSD extraction, output saving, and progress visualization
    \item Estimated progress: Approximately 50--55\%
\end{itemize}
...
These thresholds were kept fixed and non-tuned to preserve a consistent preprocessing baseline across all subjects.

The figure below summarizes epoch counts for \texttt{sub-001} under non-overlap (0\%) and 50\% overlap, showing total, retained, rejected, amplitude-rejected, and high-frequency-rejected epochs side by side. The overlap setting increases the total number of epochs, while the same rejection rules are applied.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{results/figures/slide_assets/04_artifact_rejection_summary.png}
\caption{Artifact rejection summary for \texttt{sub-001}: 0\% vs 50\% overlap}
\end{figure}
...
\subsection{Execution Summary of Implemented Pipeline}
To validate reproducibility, the Phase 1 notebooks were executed in sequence:
\begin{itemize}
    \item \texttt{01\_dataset\_overview.ipynb}
    \item \texttt{02\_preprocessing\_single\_sample.ipynb}
    \item \texttt{03\_feature\_extraction\_single\_sample.ipynb}
    \item \texttt{04\_feature\_extraction\_all\_subjects.ipynb}
    \item \texttt{05\_progress\_visualizations.ipynb}
\end{itemize}

All five notebooks completed successfully and generated the expected outputs (metadata files, subject-wise PSD files, overlap comparison summaries, and updated report figures).

\subsection{Overlap versus Non-overlap Observation}
After implementing both segmentation settings, the 50\% overlap pipeline produced approximately double the average retained epochs per subject (325.86) compared with the non-overlap baseline (163.18), while processing the same 88 subjects successfully. This corresponds to nearly a 99.7\% increase in average retained epochs per subject.

In practical terms, both settings preserve the same physiological recording context and preprocessing rules, but the 50\% overlap setting increases sample availability by using a 2-second shift between consecutive 4-second epochs. This is important for the next project phase because it enables larger model input sets without requiring additional data collection.

\subsection{Output Completeness and Data Integrity}
The generated outputs confirm that the Phase 1 pipeline is complete and internally consistent:
\begin{itemize}
    \item \textbf{Non-overlap feature files:} 88 subject-wise \texttt{.npz} files stored under
    \path{data/processed/psd_features/}.
    \item \textbf{50\% overlap feature files:} 88 subject-wise \texttt{.npz} files stored under
    \path{data/processed/psd_features_overlap50/}.
    \item \textbf{Summary files:} both
    \path{data/metadata/psd_feature_summary.csv} and
    \path{data/metadata/psd_feature_summary_overlap50.csv}
    are available and populated for all matched subjects.
    \item \textbf{Failure count:} 0 failures in both processing settings
\end{itemize}

These checks indicate that the pipeline is not only functionally correct but also robust at full-dataset scale.

\subsection{Subject-level Variability}
The retained epoch range varies across subjects in both settings:
\begin{itemize}
    \item Non-overlap: minimum 1, maximum 315 retained epochs
    \item 50\% overlap: minimum 5, maximum 630 retained epochs
\end{itemize}

This variability is expected in real clinical EEG due to differences in recording quality and artifact burden across participants. Importantly, despite this variability, all subjects still produced valid PSD outputs, which supports downstream comparative modeling feasibility.
...
The retained epoch counts vary across subjects, which is expected due to different artifact levels in real EEG recordings. However, the pipeline still produced valid PSD features for every processed subject under both segmentation settings. This indicates that the preprocessing and feature extraction modules are stable, reproducible, and suitable for transition into Phase 2 model development.

Overall, the Phase 1 outcome is technically complete: data ingestion, quality-controlled preprocessing, overlap comparison, and standardized Welch PSD feature generation have all been verified and documented with saved outputs and figures. This provides a strong and traceable foundation for the comparative CNN experiments planned in the next phase.
...
\begin{itemize}
    ...
    \item implement CNN models: EEGNet, ShallowConvNet, DeepConvNet, SCCNet, and FBCNet;
